...
- [ ] Feedback system (footer + feedback form + Resend email integration)

## Feedback System

Add a footer and feedback form so users can easily report bugs, request features, or share feedback.

### Phase 1: Footer

- [ ] Add `Footer` component to `app/root.tsx` (visible on all pages)
    - Left side: "Yoto Sync" brand + tagline
    - Right side: "Feedback" button, GitHub link, "Made by Brad Garropy" credit
    - Styled with `border-t bg-background`, consistent with header
    - Responsive layout (stacks on mobile)
- [ ] Update `App` component with flex column + `min-h-screen` for sticky footer
- [ ] Remove `min-h-screen` from individual route pages

### Phase 2: Feedback Dialog

- [ ] Add shadcn `Textarea` component (`app/components/ui/textarea.tsx`)
- [ ] Create `FeedbackDialog` component (`app/components/FeedbackDialog.tsx`)
    - Category select: Bug Report / Feature Request / General Feedback
    - Message textarea (required)
    - Email input (optional)
    - Submit button (placeholder toast, no backend yet)
- [ ] Wire Footer "Feedback" button to open the dialog

### Phase 3: Resend Integration

- [ ] Install `resend` dependency
- [ ] Create API route (`app/routes/api.feedback.ts`)
    - Validate form data
    - Send email via Resend API
    - Return JSON response
- [ ] Update `FeedbackDialog` to POST via `useFetcher` to `/api/feedback`
- [ ] Add `RESEND_API_KEY` env var via `wrangler secret put`
